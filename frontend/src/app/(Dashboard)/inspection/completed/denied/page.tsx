...
export default function Page() {
  ...
  const handleUnassignInspector = async (requestId: string) => {
    ...
    try {
      ...
      // Refresh the list
      const response = await axios.get(`${apiUrl}/inspections/api/v1/admin-requests/?status=Denied`, { headers });
  ...
  useEffect(() => {
    const fetchInspectionRequests = async () => {
      ...
      try {
        ...
        setError(err?.response?.data?.detail || err?.message || "Failed to fetch denied inspections.");
        showErrorToast(err, "Denied inspections");
        setData([]);
      } finally {
        setLoading(false);
      }
