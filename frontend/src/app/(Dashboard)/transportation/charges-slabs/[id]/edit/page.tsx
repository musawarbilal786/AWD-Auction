...
export default function EditChargesSlabPage() {
  ...
  useEffect(() => {
    const fetchSlabData = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${apiUrl}/transportation/api/v1/charges-slab/${slabId}/`, { headers });
        const slabData = response.data;

        // Map API data to form fields
        form.setFieldsValue({
          slabName: slabData.name || slabData.slab_name,
          rangeStart: slabData.km_range_start || slabData.range_start,
          rangeEnd: slabData.km_range_end || slabData.range_end,
          buyerCharges: slabData.buyer_charges_per_km || slabData.buyer_charges,
          transporterCharges: slabData.transporter_charges_per_km || slabData.transporter_charges,
          status: slabData.status === 1 ? "Active" : "Inactive",
        });
      } catch (error: any) {
        showErrorToast(error, "Fetching charges slab");
      } finally {
        setFetching(false);
      }
    };
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      await axios.patch(
        `${apiUrl}/transportation/api/v1/charges-slab/${slabId}/`,
        payload,
        { headers }
      );

      showSuccessToast(COMMON_SUCCESS_MESSAGES.UPDATED, "Charges Slab");
      router.push('/transportation/charges-slabs');
    } catch (error: any) {
      showErrorToast(error, "Charges slab update");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (fetching) {
    return (
      <div>
        <Breadcrumbs items={[{ label: "Transportation", href: "/transportation" }, { label: "Charges Slabs", href: "/transportation/charges-slabs" }, { label: "Edit" }]} />
        <div className="bg-white rounded-xl shadow-md p-6">
          <div className="text-center py-8 text-gray-500">Loading charges slab data...</div>
        </div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        <div className="flex justify-between items-center mb-6">
          {/* <h2 className="text-2xl font-bold text-sky-700">Transportation Charges Slabs <span className="text-black font-normal">/ Edit</span></h2> */}
          <Button 
            type="primary" 
